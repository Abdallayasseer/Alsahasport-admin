...
const Sessions = () => {
  ...
  const filteredSessions = sessions.filter((session) => {
    ...
    return (
      ...
      (session.userId?._id &&
        session.userId._id.toLowerCase().includes(searchLower)) ||
      (session.userId?.username &&
        session.userId.username.toLowerCase().includes(searchLower))
    );
  });
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-400">
            Monitor real-time user activity across devices.
          </p>
        </div>
        <Button
          onClick={() => refetch()}
              ...
              {isLoading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-6 py-5">
                      <div className="h-5 w-24 rounded bg-zinc-800 animate-pulse"></div>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5">
                      <div className="h-5 w-12 rounded bg-zinc-800 animate-pulse"></div>
                    </td>
                  </tr>
                ))
                  ...
                  <tr
                    ...
                    className={`group transition-all duration-200 hover:bg-white/[0.03] ${
                      isRecent(session.lastActive)
                        ? "bg-indigo-500/[0.03] border-l-2 border-l-indigo-500"
                        ...
                        <div>
                          {session.userId?.username ? (
                            ...
                            <div className="font-mono text-zinc-400 text-xs">
                              Code: ...
                              {session.userId?._id?.slice(-6) || "Unknown"}
                            </div>
                          )}
                    ...
                    <td className="whitespace-nowrap px-6 py-5">
                      <div className="flex items-center gap-2 text-sm text-zinc-400">
                        ...
                        <div className="flex flex-col">
                          <span
                            className="text-zinc-300 text-xs font-medium max-w-[150px] truncate"
                      ...
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-sm text-zinc-300">
                          ...
                          <span
                            ...
                            title="Best Guess Public IP"
                          >
                            {session.clientPublicIp ||
                              session.proxyDetectedIp ||
                              session.ipAddress ||
                              "Unknown"}
                          </span>
                        </div>
                        ...
                        <div className="flex flex-col gap-0.5 ml-6">
                          ...
                          <div className="mb-1">
                            <span
                              className={`inline-flex items-center rounded-sm px-1.5 py-0.5 text-[0.6rem] font-medium ring-1 ring-inset ${
                                session.ipConfidence === "HIGH"
                                  ? "bg-emerald-400/10 text-emerald-400 ring-emerald-400/20"
                                  : session.ipConfidence === "MEDIUM"
                                  ? "bg-yellow-400/10 text-yellow-400 ring-yellow-400/20"
                                  : "bg-red-400/10 text-red-400 ring-red-400/20"
                              }`}
                            >
                              {session.ipConfidence || "LOW"} CONFIDENCE
                            </span>
                          </div>
                          {/* Proxy IP if different or interesting */}
                          {session.proxyDetectedIp &&
                            session.proxyDetectedIp !==
                              session.clientPublicIp && (
                              <span className="text-[10px] text-zinc-500 font-mono">
                                Proxy: {session.proxyDetectedIp}
                              </span>
                            )}
                          {/* Client Claimed IP if interesting */}
                          {session.clientPublicIp && (
                            <span className="text-[10px] text-zinc-500 font-mono">
                              Client: {session.clientPublicIp}
                            </span>
                          )}
                        </div>
                      </div>
