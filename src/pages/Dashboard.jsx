...
const StatCard = ({ title, value, change, gradient, icon: Icon }) => (
  <Card className="relative overflow-hidden group hover:border-white/20 transition-all duration-300">
    ...
    <div className="relative z-10 flex flex-col h-full justify-between">
      <div>
        <div className={`inline-flex p-3 rounded-xl bg-gradient-to-br ${gradient} bg-opacity-10 mb-4 shadow-lg shadow-black/20 ring-1 ring-white/5`}>
...
const TimelineItem = ({ title, time, status, type, details, onCopy }) => (
  <div className="relative pl-6 pb-6 border-l border-white/5 last:pb-0 group/item">
    <div className={`absolute -left-[5px] top-1 h-2.5 w-2.5 rounded-full ring-4 ring-zinc-950 ${
...
const PasswordModal = ({ isOpen, onClose, onConfirm, isLoading }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="w-full max-w-md bg-zinc-900 border border-white/10 rounded-xl shadow-2xl p-6 relative animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex flex-col items-center mb-6 text-center">
                    ...
                    <p className="text-sm text-zinc-400 mt-1">
                        Please enter your admin password to reveal and copy this activation code.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        >
                            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Reveal Code'}
                        </Button>
                    </div>
                </form>
            </div>
...
const Dashboard = () => {
  ...
  const stats = statsData || { totalCodes: 0, activeSessions: 0, serverLoad: 0, revenue: 0 };
  ...
  const handleRevealConfirm = async (password) => {
    ...
    setIsRevealing(true);
    try {
        const { data } = await api.post(`/admin/codes/${selectedCodeId}/reveal`, { password });
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-white/5">
        <div>
            <div className={`flex items-center gap-2 ${isSystemOnline ? 'text-emerald-400' : 'text-red-400'} text-xs font-bold tracking-wider uppercase mb-2`}>
            <span className="relative flex h-2 w-2">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${isSystemOnline ? 'bg-emerald-400' : 'bg-red-400'} opacity-75`}></span>
              <span className={`relative inline-flex rounded-full h-2 w-2 ${isSystemOnline ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
            </span>
            {isSystemOnline ? 'System Online' : 'System Offline'}
          </div>
          <h1 className="text-4xl font-bold text-white tracking-tight">
            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-400">{user?.username || 'Admin'}</span>
          </h1>
          <p className="mt-2 text-zinc-400 max-w-xl">
            Here's what's happening in your workspace today. Server load is stable and user activity is trending up.
          </p>
        </div>
        <div className="text-right">
          <div className="text-4xl font-light text-white tracking-tighter">{timeString}</div>
          <div className="text-sm font-medium text-zinc-500 uppercase tracking-widest mt-1">{dateString}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 min-h-[400px] flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-lg font-bold text-white">Weekly Code Generation</h3>
              <p className="text-sm text-zinc-500">Performance metrics over the last 7 days</p>
            </div>
          </div>
          
          <div className="flex-1 flex items-end justify-between gap-2 px-4 pb-4">
            {chartData.map((d, i) => (
              <div key={i} className="w-full bg-zinc-800/50 rounded-t-lg relative group h-full flex flex-col justify-end">
                <div 
                  ...
                  style={{ height: `${d.value > 0 ? (d.value / Math.max(...chartData.map(c => c.value), 10)) * 100 : 5}%` }}
                >
        ...
        <Card className="flex flex-col">
          ...
          <div className="space-y-1 overflow-y-auto max-h-[320px] scrollbar-thin scrollbar-thumb-zinc-800">
             {activity.length === 0 ? (
               <div className="text-zinc-500 text-sm py-4 text-center">No recent activity</div>
             ) : (
